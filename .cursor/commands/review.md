...
7. **Lint and build** – Ensure lint and build complete successfully with no errors and all warnings resolved:
   - Run `npm run lint` in the app directory to catch ESLint errors and warnings (not just TypeScript diagnostics). The `read_lints` tool may miss some ESLint rules, so always verify with the actual lint command.
