...
## View Toggle (Shared with Account Balances)

### US-23: View Toggle Button
**As a user**, I want a "Switch to Account Balances" button below the summary grid so I can toggle to see my account balances for the month.

### US-24: Button Bar Layout
**As a user**, I want the view toggle button on the left, with the "Edit Allocations" button to its right (when applicable), so I have a clear action bar.

### US-25: View Persistence in URL
**As a user**, I want the selected view to be saved in the URL query parameter (`?view=categories`) so that when I reload the page, I stay on the same view.

### US-26: View Persistence Across Navigation
**As a user**, I want the selected view to be remembered in the app context when I navigate to other pages and come back, so I don't have to re-select it each time.
