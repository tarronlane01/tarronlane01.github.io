...
import { ensureMonthsFreshAndRecalculateBalances } from '@budget/data/mutations/month/ensureMonthsFresh'
...
export function useEnsureBalancesFresh(enabled: boolean = true, options: UseEnsureBalancesFreshOptions = {}) {
  ...
  useEffect(() => {
    // Reset hasRunRef when budget changes (do this FIRST, before any early returns)
    // This handles direct budget switches (A -> B) where selectedBudgetId never becomes null
    if (selectedBudgetId !== lastBudgetIdRef.current) {
      hasRunRef.current = false
      lastBudgetIdRef.current = selectedBudgetId
    }
