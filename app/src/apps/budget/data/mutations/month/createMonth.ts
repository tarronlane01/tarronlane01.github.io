...
import type { MonthDocument, FirestoreData, CategoryMonthBalance, AccountMonthBalance, MonthMap } from '@budget/types'
...
import type { MonthQueryData } from '@budget/data/queries/month/readMonth'
import { getEndBalancesFromMonth } from '@budget/data/queries/month/getEndBalancesFromMonth'
import { updateCacheWithSingleMonth } from '@budget/data/recalculation/monthMapHelpers'
import { convertMonthBalancesFromStored } from '@budget/data/converters/monthBalances'
...
export async function createMonth(
  ...
  // Build account_balances with correct start_balance from previous month
  ...
  // Only persist start_balance for months at/before the window start
  // Months after the window compute start_balance on-the-fly from previous month
  ...
  const docToWrite: FirestoreData = {
    ...
    transfers: [],
    adjustments: [],
    account_balances: isAtOrBeforeWindow
      ? accountBalances
      : accountBalances.map(({ account_id }) => ({ account_id, start_balance: 0 })),
    category_balances: isAtOrBeforeWindow
      ? categoryBalances
      : categoryBalances.map(({ category_id, allocated }) => ({ category_id, start_balance: 0, allocated })),
    are_allocations_finalized: false,
    created_at: nowIso,
    updated_at: nowIso,
  }
