...
import { doc, collection, query, where, getDoc, getDocs, onSnapshot, type Query, type DocumentReference } from 'firebase/firestore'
 ...
 */
export function subscribeToDoc<T = FirestoreData>(
  collectionPath: string,
  docId: string,
  source: string,
  onData: (data: T | null, exists: boolean) => void,
  ...
  return onSnapshot(
    ...
    (snapshot) => {
      const exists = snapshot.exists()
      const data = exists ? (snapshot.data() as T) : null
      logFirebase('SUBSCRIBE', path, source, 1, exists)
      onData(data, exists)
    },
    (error) => {
      console.error(`[Firestore] SUBSCRIBE error for ${path}:`, error)
      onError?.(error)
    }
  )
}
