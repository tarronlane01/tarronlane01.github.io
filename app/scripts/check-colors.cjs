#!/usr/bin/env node
/**
 * Enforces that all colors come from src/shared/constants/colors.ts (and index.css for variable definitions).
 ...
 *
 * Run from app directory: node scripts/check-colors.cjs
 */
...
const COLORS_TS = path.join(SRC_DIR, 'shared', 'constants', 'colors.ts')
