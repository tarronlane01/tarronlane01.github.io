...
# Combined review checks for the app
# Run from app directory: bash scripts/review-checks.sh
...
if [ -n "$DEEP_IMPORTS" ]; then
  ...
  echo "$DEEP_IMPORTS"
  echo ""
  echo "   Available aliases: @components, @utils, @data, @hooks, @contexts, @styles, @types, @constants, @firestore, @calculations, @budget"
  echo ""
  EXIT_CODE=1
else
