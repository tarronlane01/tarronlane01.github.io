/**
 * Account Balance Stale Helpers
 *
 * Functions to mark account balances as stale at budget and month levels.
 * Similar to categoryBalanceStaleHelpers but for account balances.
 */
...
// ============================================================================
// BUDGET-LEVEL ACCOUNT BALANCES SNAPSHOT STALE HELPERS
// ============================================================================

/**
 * Mark budget account snapshot stale in CACHE only.
 * Use this in onMutate for instant UI feedback.
 */
export function markAccountBalancesSnapshotStaleInCache(budgetId: string): void {
  const budgetKey = queryKeys.budget(budgetId)
  const budgetData = queryClient.getQueryData<BudgetData>(budgetKey)

  if (budgetData?.accountBalancesSnapshot && !budgetData.accountBalancesSnapshot.is_stale) {
    queryClient.setQueryData<BudgetData>(budgetKey, {
      ...budgetData,
      accountBalancesSnapshot: {
        ...budgetData.accountBalancesSnapshot,
        is_stale: true,
      },
    })
  }
...
/**
 * Mark budget account snapshot stale in FIRESTORE only.
 ...
 */
export async function markAccountBalancesSnapshotStaleInFirestore(budgetId: string): Promise<void> {
  try {
    ...
    if (cachedBudget?.accountBalancesSnapshot?.is_stale) {
    ...
    const { exists, data } = await readDoc<FirestoreData>(
      'budgets',
      budgetId,
      'checking if budget account snapshot needs stale flag'
    )
    if (exists && data && data.account_balances_snapshot && !data.account_balances_snapshot.is_stale) {
      await writeDoc(
        'budgets',
        budgetId,
        {
          ...
          account_balances_snapshot: {
            ...data.account_balances_snapshot,
            is_stale: true,
          },
        },
        'marking budget account balances snapshot as stale (income or expense changed)'
      )
      ...
      if (cachedBudget) {
        queryClient.setQueryData<BudgetData>(budgetKey, {
          ...cachedBudget,
          accountBalancesSnapshot: cachedBudget.accountBalancesSnapshot
            ? { ...cachedBudget.accountBalancesSnapshot, is_stale: true }
            : null,
        })
      }
    }
  } catch (err) {
    console.error('Error marking account balances snapshot stale in Firestore:', err)
  }
}

// ============================================================================
// MONTH-LEVEL ACCOUNT BALANCES STALE HELPERS
// ============================================================================

/**
 * Mark a specific month's account_balances as stale in CACHE only.
 * Use this in onMutate for instant UI feedback.
 */
export function markMonthAccountBalancesStaleInCache(
  budgetId: string,
  year: number,
  month: number
): void {
  ...
  if (monthData?.month && !monthData.month.account_balances_stale) {
    queryClient.setQueryData<MonthQueryData>(monthKey, {
      ...monthData,
      month: {
        ...monthData.month,
        account_balances_stale: true,
      },
    })
  }
}
...
/**
 * Mark a specific month's account_balances as stale in FIRESTORE only.
 * Use this in mutationFn. Only writes if not already stale.
 * Uses fetchQuery to leverage cache when available.
 */
export async function markMonthAccountBalancesStaleInFirestore(
  budgetId: string,
  year: number,
  month: number
): Promise<void> {
  ...
  try {
    const result = await queryClient.fetchQuery<MonthQueryData>({
      ...
      queryFn: async () => {
        const { exists, data } = await readDoc<FirestoreData>(
          'months',
          monthDocId,
          `checking if ${year}/${month} account balances need stale flag`
        )
        if (!exists || !data) {
    ...
    if (result.month.account_balances_stale) {
      return
    }
    ...
    await writeDoc(
      ...
      `marking ${year}/${month} account balances as stale`
    )
    ...
    console.error('Error marking month account balances stale in Firestore:', err)
  }
}

/**
 * Mark all future months' account_balances as stale in CACHE only.
 * Use this in onMutate for instant UI feedback.
 */
export function markFutureMonthsAccountBalancesStaleInCache(
  budgetId: string,
  afterYear: number,
  afterMonth: number
): void {
...
/**
 * Mark all future months' account_balances as stale in FIRESTORE only.
 * Use this in mutationFn. Only writes if not already stale.
 *
 ...
 */
export async function markFutureMonthsAccountBalancesStaleInFirestore(
  budgetId: string,
  afterYear: number,
  afterMonth: number
): Promise<void> {
  try {
    ...
    const monthsResult = await queryCollection<{
      ...
      'months',
      `finding months >= ${afterYear} to mark account balances stale`,
      [
    ...
    for (const doc of monthsResult.docs) {
      ...
      if (isAfter && !doc.data.account_balances_stale) {
        await writeDoc(
          'months',
          doc.id,
          {
          ...
          `marking ${docYear}/${docMonth} account balances as stale (earlier month was edited)`
        )
    ...
    console.error('Error marking future months account balances stale in Firestore:', err)
  }
}
